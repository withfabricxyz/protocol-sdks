...
const helpers = {
  ...
  async withdrawYield(config: DepositRequest & { erc20?: boolean }): Promise<TransactionReceipt> {
...
test('[ETH] contributes to a campaign', async ({ campaignAddress }: TCampaignTestContext) => {
...
test('[ETH] transfers balance from a campaign to the campaign recipient', async ({ campaignAddress }: TCampaignTestContext) => {
...
test('[ETH] yields funds to a campaign', async ({ campaignAddress }: TCampaignTestContext) => {
...
test('[ETH] withdraws funds from a campaign', async ({ campaignAddress }: TCampaignTestContext) => {
